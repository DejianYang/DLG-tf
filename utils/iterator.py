...
class DialogIterator(object):
    def __init__(self, dialog_data,
        ...
        if not self._infer:
            ...
            ctx_resp_pairs = self.generate_context_response_pairs(self._dialog_data)
    ...
    def _dynamic_padding_multi_turn(self, batch_data):
    ...
    def next_batch(self):
        ...
        if not self._infer:
            ...
            for batch_data in batch_samples:
                ...
                resp_input_data = []
                resp_output_data = []
                for (ctx, resp) in batch_data:
                    ctx_data.append(ctx)
                    resp_input_data += [[self._sos_idx] + resp]
                    resp_output_data += [resp + [self._eos_idx]]

                pad_ctx, pad_ctx_length = self._dynamic_padding_multi_turn(ctx_data)
